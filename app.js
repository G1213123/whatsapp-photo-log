...
async function fetchMessagesTill(chat, time, period) {
    let batch = 0
    ...
    if (period == 0 ) {var endtime = Date.now()} else {
        var endtime = new Date(time)
        endtime.setDate(starttime.getDate() + period)
    }
    
    while (lastMessageTime > starttime) {
        messages = await fetchMessages()
        lastMessageTime = new Date(messages[0].timestamp * 1000)
        //starttime = new Date(time)
    }
    //filter message in time range
    final_messages = messages.filter(msg => new Date(msg.timestamp * 1000) > starttime && new Date(msg.timestamp * 1000) < endtime)
...
client.on('ready', async () => {
    ...
    fetchMessagesTill(chat, process.env.FETCH_MSG_TIME, parseInt(process.env.Fetch_MSG_DAY)).then(async (messages) => {
